...
RECO uses a system prompt fed to GPT-4o, allowing it to act a a virtual doctor collecting patient information in a routine appointment. A synthetic patient bot is directed via a system prompt to represent a RECO patient, utilizing real-world, anonymized data to mimic actual heart failure patients. These two bots then participate in simulated dialogues, and the transcripts of these conversations are evaluated and fed to the summarization engine.
